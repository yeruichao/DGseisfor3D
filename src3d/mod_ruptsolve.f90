...
!--------------------------------------------------------------------
	...
	USE mpi
    use para_mod,     only : rk4a,rk4b,rk4c,convtest,rupt_gamma,&
                             myrank,grav,def_pOrder
    ...
    use friction_mod, only : RSfric_F,RSfric_dFdN,RSfric_dFdS,&
                             ...
                             RSfric_invG,sigmin
    use parallel_mod, only : domain_exchange,domain_exchange1_vec

    implicit none
!    include 'mpif.h'

    private :: ntH,ntG,ntS,ntY,ntZ,ntS0,ntS1
    ...
    public :: rupt_alpha_V, rupt_alpha_E, rupt_alpha_edge, rupt_on

    real(kind=rkind),allocatable,target :: &
                ntH(:,:),ntG(:),ntS(:),ntS0(:),ntS1(:),ntY(:),ntZ(:)
    real(kind=rkind),allocatable :: ntH0(:,:),ntH1(:,:)
...
!    type(pointer_mat_real),pointer :: pA(:,:)
    ...
    real(kind=rkind),parameter :: &
        rupt_alpha_edge=1.0d-1, &
!        rupt_alpha_V = 1.0d0, rupt_alpha_E=1.0d0
        rupt_alpha_V = 1.0d1, rupt_alpha_E=0.0d-1
...
!         if penalty for E is required
    ...
    real(kind=rkind),allocatable :: wt(:)
!--------------------------------------------------------------------
    contains
!--------------------------------------------------------------------
...
subroutine rupt_Newton_allocate(pNp,Nfp,rupt)
    ...
    allocate(wt(rupt%Nface*(Nfp+3*(def_pOrder+1))*3))
    ...
    allocate(ntH0(3*Nfp,3*Nfp))
    allocate(ntH1(3*Nfp,3*Nfp))
...
subroutine rupt_matrix_allocate(pNp,Nfp,M)
    ...
    allocate(M%g0m(Nfp*3,pNp*3));M%g0m=0d0
    allocate(M%g0p(Nfp*3,pNp*3));M%g0p=0d0
...
!---------------------------------------------------------------------!
        ...
        Dm(:,:,1)=invJ(1)*matrix%Drw&
                 +invJ(2)*matrix%Dsw&
                 +invJ(3)*matrix%Dtw
        Dm(:,:,2)=invJ(4)*matrix%Drw&
                 +invJ(5)*matrix%Dsw&
                 +invJ(6)*matrix%Dtw
        Dm(:,:,3)=invJ(7)*matrix%Drw&
                 +invJ(8)*matrix%Dsw&
                 +invJ(9)*matrix%Dtw
...
!---------------------------------------------------------------------!
        ...
        Dp(:,:,1)=invJ(1)*matrix%Drw+invJ(2)*matrix%Dsw+invJ(3)*matrix%Dtw
        Dp(:,:,2)=invJ(4)*matrix%Drw+invJ(5)*matrix%Dsw+invJ(6)*matrix%Dtw
        Dp(:,:,3)=invJ(7)*matrix%Drw+invJ(8)*matrix%Dsw+invJ(9)*matrix%Dtw
...
subroutine form_rupt_matrices1(pNp,Nfp,mesh,rupt,T0,matrix,dt)
    ...
    real(kind=rkind) :: &
        ...
        irtJm   (pNp*3,Nfp*3),irtJp   (pNp*3,Nfp*3),&
...
!stop
        ...
        do i=1,3
        do j=1,3
            irtJm((j-1)*pNp+1:j*pNp,(i-1)*Nfp+1:i*Nfp)=&
            irJm ((j-1)*pNp+1:j*pNp,(i-1)*Nfp+1:i*Nfp)*(1-n(j)*n(i))
            irtJp((j-1)*pNp+1:j*pNp,(i-1)*Nfp+1:i*Nfp)=&
            irJp ((j-1)*pNp+1:j*pNp,(i-1)*Nfp+1:i*Nfp)*(1-n(j)*n(i))
        enddo
        enddo
...
!!---------------------------------------------------------------------!
        ...
        rupt%M(iface)%AVmVt= rupt_alpha_V*dt*irtJm
        rupt%M(iface)%AVpVt=-rupt_alpha_V*dt*irtJp
...
subroutine rupt_Newton(pNp,Nfp,mesh,rupt,dt,W,dT0,matrix)
    integer,intent(in) :: pNp,Nfp
    type(tetmesh_geometry) :: mesh
    type(rupture) :: rupt
    type(matrices) :: matrix
    ...
    real(kind=rkind),parameter :: delt(3,3)=&
        reshape((/1d0,0d0,0d0,0d0,1d0,0d0,0d0,0d0,1d0/),&
		shape(delt))
    integer :: iter,ipiv(3*Nfp),info,Niter,iterM(1)
!    integer,parameter :: itermax=60
    ...
    real(kind=rkind) :: Etest(Nfp*9),Vtest(Nfp*3),dVtest(pNp*3),&
        dVtest1(Nfp*3)
    real :: xtmp,ytmp,ztmp
    real(kind=rkind) :: M2D(Nfp,Nfp),tau2m(Nfp*3),tau2p(Nfp*3),&
        sigma0(Nfp),dsig(Nfp)
    integer :: Nfpm1
    real(kind=rkind),parameter :: gama=1d0,mgSmin=1d-16
    real(kind=rkind) :: dbgF0(Nfp),dbgS0(Nfp),&
        dbgVt0(Nfp*3),dbgTf0(Nfp*3),dbgsig0(Nfp),&
        dbgTf1(Nfp*3),dbgVt1(Nfp*3),dbgsig1(Nfp)

!    Nfpm1=(def_pOrder+1)*def_pOrder/2
!    M2D=matmul(&
!        transpose(matrix%iV2D(1:Nfpm1,:)),matrix%iV2D(1:Nfpm1,:))
    M2D=matrix%M2D
...
!stop
    ...
    ! form Z
        call rupt_map_asign_V(pNp,W%U,hUm,vmapm,0d0,1d0,'s')
        call rupt_map_asign_V(pNp,W%U,hUp,vmapp,0d0,1d0,'s')
        call rupt_map_asign_E(pNp,W%E,hEm,vmapm,0d0,1d0,'s')
        call rupt_map_asign_E(pNp,W%E,hEp,vmapp,0d0,1d0,'s')
        call rupt_map_asign_V(pNp,W%V,hVm,vmapm,0d0,1d0,'s')
        call rupt_map_asign_V(pNp,W%V,hVp,vmapp,0d0,1d0,'s')
...
!        hTf = hTf - 0.5d0*matmul(rupt%M(iface)%g0m,hUm)
!        hTf = hTf + 0.5d0*matmul(rupt%M(iface)%g0p,hUp)

!        tau2m = matmul(rupt%M(iface)%g0m,hUm)
!        tau2p = matmul(rupt%M(iface)%g0p,hUp)
!        hVm = hVm + matmul(rupt%M(iface)%irJm,tau2m)*dt
!        hVp = hVp - matmul(rupt%M(iface)%irJp,tau2p)*dt
!        hTf = hTf + 0.5d0*tau2m
!        hTf = hTf - 0.5d0*tau2p
...
!!!!!!!! treatment for sigma <= 0 !!!!!!!!!!!!!!!!!!!!!!!!!!!
        if(any(rupt%sigma(map).le.0d0))then
!            pSig=-0.5d0*matmul(rupt%M(iface)%BLRCm,pEm)&
!                 -0.5d0*matmul(rupt%M(iface)%BLRCp,pEp)+hSig
!            rupt%sigma(map) = pSig
            cycle
        endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!        mgS=sqrt(pS(1)%p**2+pS(2)%p**2+pS(3)%p**2)
        ...
        iterM(1)=itermax;flag=.true.;res=0d0
...
1092    continue
    ...
    ! start Newton iteration
        ...
        ! compute Ybar
            ...
            pSig=pSig+rupt%sigma0(map)!+sigma0
!            print*,'rupture sigma0',rupt%sigma0(map)
!            print*,'sigma0',sigma0
!            stop

        ! compute friction derivatives
            do n=1,Nfp
!                mgS(n)=max(mgS(n),mgSmin)
!                if(pSig(n).le.0d0)then
!                    pSig(n)=0d0
!                    pS(1)%p(n) = W%V%x(mapp(n)) - W%V%x(mapm(n))
!                    pS(2)%p(n) = W%V%y(mapp(n)) - W%V%y(mapm(n))
!                    pS(3)%p(n) = W%V%y(mapp(n)) - W%V%y(mapm(n))
!                    mgS(n)=sqrt(pS(1)%p(n)**2 &
!                               +pS(2)%p(n)**2 &
!                               +pS(3)%p(n)**2)
!                endif
                a0  = rupt%a (head+n)
...
!                    pG(j)%p=pG(j)%p+matmul(transpose(tmp1),&
!                        matmul(M2D,rFl(:,l)))
                enddo
                do j=1,3
                    do i=1,3
...
!                            matmul(M2D,ntJac(:,:,i)) )
            ...
            enddo
!! regularization !!!!
            if(any(dsig.lt.0d0))then
                do j=1,3
                    pG(j)%p=pG(j)%p+gama*matmul(transpose(pA(4,j)%p),dsig)
                    do i=1,3
                        pH(i,j)%p=pH(i,j)%p+gama*&
                            matmul(transpose(pA(4,j)%p),pA(4,i)%p)
                    enddo
                enddo
            endif
...
!        if(iter.gt.itermax.and.flag)then
!            iterM=minloc(res)
!            call rupt_map_asign_V(Nfp,rupt%Vt,ntS,map,0d0,1d0,'s')
!            flag=.false.
!            goto 1092
...
!        if( .false. )then
!        if( any(pPsi.le.1d-1) )then
        if( any(ntS.ne.ntS).or.any(mgS.gt.1d0) )then
!        if(iter.ge.2)then
...
!            print*,res1
            ...
            print*,'sigma_min'
            print*,smp4
            call DGEEV('N','N',Nfp*3,ntH0,Nfp*3,&
                ntG,ntS,ntZ1,1,ntZ2,1,ntY1,30*pNp,i)
            if(i.ne.0)print*,'eigen calc unsuccess'
            print*,'EigenValue of NtH0'
            print*,ntG
            call DGEEV('N','N',Nfp*3,ntH1,Nfp*3,&
                ntG,ntS,ntZ1,1,ntZ2,1,ntY1,30*pNp,i)
            if(i.ne.0)print*,'eigen calc unsuccess'
            print*,'EigenValue of NtH1'
            print*,ntG
            print*,'displacement'
            print*,hUm,hUp
            print*,'tau2m'
            print*,tau2m
            print*,'irJtau2m'
            print*,size(rupt%M(iface)%irJm),size(tau2m)
            print*,matmul(rupt%M(iface)%irJm,tau2m)
            print*,'tau2p'
            print*,tau2p
            print*,'irJtau2p'
            print*,size(rupt%M(iface)%irJp),size(tau2p)
            print*,matmul(rupt%M(iface)%irJp,tau2p)
            print*,'mgS'
            print*, mgS
            print*,'psi'
            print*,pPsi
...
!endif
    ! update wavefield
        ...
        call rupt_map_asign_E(pNp,W%E,pEm,vmapm,0d0,1d0,'v')
        call rupt_map_asign_E(pNp,W%E,pEp,vmapp,0d0,1d0,'v')
        call rupt_map_asign_V(pNp,W%V,pVm,vmapm,0d0,1d0,'v')
        call rupt_map_asign_V(pNp,W%V,pVp,vmapp,0d0,1d0,'v')
...
!        call rupt_map_asign_V(Nfp,rupt%tauf,tau2m-tau2p,map,0d0,1d0,'v')
        call rupt_map_asign_V(Nfp,rupt%tauf,rFl(1:,1),map,0d0,1d0,'v')
...
!if(myrank.eq.0)write(*,'(I1\ )')iter
!if(myrank.eq.0.and.iter.gt.itermax)write(*,'(I1\ )')iter
!if(myrank.eq.0.and.iterM(1).ne.itermax)write(*,'(I1\ )')iter
!endif

end subroutine rupt_Newton

subroutine rupt_edge_pflux(pNp,Nfp,Nep,subdomain,mesh,rupt,V,fV,&
        matrix,alpha)
    integer,intent(in) :: pNp,Nfp,Nep 
    type(tetmesh_geometry) :: mesh
    ...
    real(kind=rkind) :: fv(1)
    integer :: Nhface,fh1,fh2,fh3,iface,itet,itri,map(Nfp),i,j,k,&
        h1,h2,h3
    Nhface=rupt%Nhface
    do iface=1,Nhface
        itet=rupt%T2E(1,iface)
        itri=rupt%T2E(3,iface)
        map(rupt%perm(:,1,iface))=&
            mesh%vmapM((itri-1)*Nfp+1:itri*Nfp)+(itet-1)*pNp
        rupt%Vm%x((iface-1)*Nfp+1:iface*Nfp)=V%x(map)
        rupt%Vm%y((iface-1)*Nfp+1:iface*Nfp)=V%y(map)
        rupt%Vm%z((iface-1)*Nfp+1:iface*Nfp)=V%z(map)
    enddo
    call domain_exchange1_vec(subdomain,rupt%SRtype,rupt%Vm)
    fh1=Nfp*3*Nhface;fh2=fh1+Nep*3*Nhface;fh3=fh2+Nep*3*Nhface
    i=0
    do iface=1,Nhface
        do j=1,3
            do k=1,Nep
                i=i+1
                wt(fh1+i)=(rupt%Vm%x(rupt%MmapP(i))&
                    -rupt%Vm%x(rupt%MmapM(i)))&
                    *rupt%EscalM(j,iface)
                wt(fh2+i)=(rupt%Vm%y(rupt%MmapP(i))&
                    -rupt%Vm%y(rupt%MmapM(i)))&
                    *rupt%EscalM(j,iface)
                wt(fh3+i)=(rupt%Vm%z(rupt%MmapP(i))&
                    -rupt%Vm%z(rupt%MmapM(i)))&
                    *rupt%EscalM(j,iface)
            enddo
        enddo
    enddo
    call DGEMM('n','n',Nfp,Nhface*3,Nep*3,alpha,&
        matrix%Lift2D,Nfp,wt(fh1+1:),Nep*3,0d0,wt(1:),Nfp)
    i=mesh%Nele*Nfp*4;j=Nfp*Nhface
    do iface=1,Nhface
        fh1=(iface-1)*Nfp;fh2=fh1+j;fh3=fh2+j
        itet=rupt%T2E(1,iface)
        itri=rupt%T2E(3,iface)
        h1=(itet-1)*Nfp*4+(itri-1)*Nfp;h2=h1+i;h3=h2+i
        fV(h1+1:h1+Nfp)=fV(h1+1:h1+Nfp)+wt(fh1+1:fh1+Nfp)
        fV(h2+1:h2+Nfp)=fV(h2+1:h2+Nfp)+wt(fh2+1:fh2+Nfp)
        fV(h3+1:h3+Nfp)=fV(h3+1:h3+Nfp)+wt(fh3+1:fh3+Nfp)
    enddo
    do iface=1,Nhface
        itet=rupt%T2E(2,iface)
        itri=rupt%T2E(4,iface)
        map(rupt%perm(:,2,iface))=&
            mesh%vmapM((itri-1)*Nfp+1:itri*Nfp)+(itet-1)*pNp
        rupt%Vp%x((iface-1)*Nfp+1:iface*Nfp)=V%x(map)
        rupt%Vp%y((iface-1)*Nfp+1:iface*Nfp)=V%y(map)
        rupt%Vp%z((iface-1)*Nfp+1:iface*Nfp)=V%z(map)
    enddo
    call domain_exchange1_vec(subdomain,rupt%SRtype,rupt%Vp)
    fh1=Nfp*3*Nhface;fh2=fh1+Nep*3*Nhface;fh3=fh2+Nep*3*Nhface
    i=0
    do iface=1,Nhface
        do j=1,3
            do k=1,Nep
                i=i+1
                wt(fh1+i)=(rupt%Vm%x(rupt%PmapP(i))&
                    -rupt%Vm%x(rupt%PmapM(i)))&
                    *rupt%EscalM(j,iface)
                wt(fh2+i)=(rupt%Vm%y(rupt%PmapP(i))&
                    -rupt%Vm%y(rupt%PmapM(i)))&
                    *rupt%EscalM(j,iface)
                wt(fh3+i)=(rupt%Vm%z(rupt%PmapP(i))&
                    -rupt%Vm%z(rupt%PmapM(i)))&
                    *rupt%EscalM(j,iface)
            enddo
        enddo
    enddo
    ...
    do iface=1,Nhface
        ...
        fV(h3+1:h3+Nfp)=fV(h3+1:h3+Nfp)+wt(fh3+1:fh3+Nfp)
    enddo
end subroutine rupt_edge_pflux
