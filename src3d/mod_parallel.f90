...
! save in Send Receive Type
    ...
    do i=1,N_DD_Conn
        ...
        enddo
    enddo
    call init_SRtype(subdomain%SRtype1,pNp,mesh%Nele,3)
...
subroutine domain_exchange1_vec(subdomain,SRtype,V)
    type(tetmesh_Domain)   :: subdomain
    type(vector_array)     :: V
    type(send_recv_pack) :: SRtype
    integer :: N_DD_Conn,ierr
    SRtype%send_req=MPI_REQUEST_NULL
    SRtype%recv_req=MPI_REQUEST_NULL
    call domain_exchange(subdomain,SRtype,V%x,&
            SRtype%send_req,SRtype%recv_req,100)
    N_DD_Conn=SRtype%host%N_DD_Conn
    if(N_DD_Conn.gt.0)&
    call MPI_Waitall(N_DD_Conn,SRtype%send_req,SRtype%sta,ierr)
    ...
    if(N_DD_Conn.gt.0)&
    call MPI_Waitall(N_DD_Conn,SRtype%recv_req,SRtype%sta,ierr)
    call domain_exchange(subdomain,SRtype,V%y,&
            SRtype%send_req,SRtype%recv_req,200)
    N_DD_Conn=SRtype%host%N_DD_Conn
    if(N_DD_Conn.gt.0)&
    call MPI_Waitall(N_DD_Conn,SRtype%send_req,SRtype%sta,ierr)
    N_DD_Conn=SRtype%gost%N_DD_Conn
    if(N_DD_Conn.gt.0)&
    call MPI_Waitall(N_DD_Conn,SRtype%recv_req,SRtype%sta,ierr)
    call domain_exchange(subdomain,SRtype,V%z,&
            SRtype%send_req,SRtype%recv_req,300)
    N_DD_Conn=SRtype%host%N_DD_Conn
    if(N_DD_Conn.gt.0)&
    call MPI_Waitall(N_DD_Conn,SRtype%send_req,SRtype%sta,ierr)
    N_DD_Conn=SRtype%gost%N_DD_Conn
...
end subroutine domain_exchange1_vec
