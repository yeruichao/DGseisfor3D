...
!       subdomain%(inele,innod)
    ...
    if(k_inout .eq. 'save')then
        ...
        if(pid.eq.0)then
            ...
            close(fid)
!            call subdomain_ckpfname(fname,fid,filename,&
!                'Matrices',-1,-1,'bin',alive)
!            open(unit=fid,file=trim(fname),access='stream',&
!                status='replace')
!            write(fid)matrix%V3D
!            write(fid)matrix%iV3D
!            write(fid)matrix%Mass
!            write(fid)matrix%Dr
!            write(fid)matrix%Ds
!            write(fid)matrix%Dt
!            write(fid)matrix%Lift
!            write(fid)matrix%V2D
!            write(fid)matrix%iV2D
!            close(fid)
        endif
...
subroutine material_file(fid,Nele,pNp,material,k_inout)
    ...
    if(k_inout .eq. 'save')then
        ...
        if(prestress)write(fid)material%T0%array
    else
